...
export const getReviews = async (req: AuthReq, res: Response) => {
...
export const addReview = async (req: AuthReq, res: Response) => {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const review = await Review.findOneAndUpdate(
        { user: userId, product: productObjectId, order: orderObjectId },
        {
          $set: { rating, comment },
      ...
      const dup = (err as any)?.code === 11000;
      if (dup) {
